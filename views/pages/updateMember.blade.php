...
{!! $form->create(['target'=>['extras.RequestHandler',['requestController'=>$requestController,'request'=>$request]],'method'=>'POST','class'=>'text-left','enctype'=>'multipart/form-data'])->run($blade)!!}
@include('extras.messages',['selector'=>$selector,'message'=>$message])
    <div class="form-group"><input type="text" name="name" class="form-control" placeholder="Jméno (Nepovinné)"/></div>
    <div class="form-group"><input type="text" name="surname" class="form-control" placeholder="Příjmení (Nepovinné)"/></div>
    <div class="form-group"><input type="date" name="age" class="form-control" min="1979-12-31" max="2015-01-02" /></div>
    <div class="form-group"><input type="text" name="location" placeholder="Město (Nepovinné)" class="form-control" /></div>
